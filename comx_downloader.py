...
# Таймауты не дают сетевым запросам зависнуть навсегда при проблемах сайта или сети.
REQUEST_TIMEOUT = (10, 60)


def safe_extract_zip(zip_file, destination):
    """Распаковывает ZIP только внутрь целевой папки, чтобы архив не мог перезаписать чужие файлы."""
...
class ComXDownloadThread(QThread):
    ...
    def _cancelled(self):
        """Проверяет мягкую отмену загрузки из UI без принудительного убийства потока."""
    ...
    def auth_via_browser(self):
        ...
        try:
            ...
            for i in range(120):
                if self._cancelled():
                    self.status.emit("Авторизация отменена")
                    break
                time.sleep(1)
                try:
        ...
        try:
            if not self._load_cookies():
                ...
                while not self._cookies_loaded:
                    if self._cancelled():
                        self.status.emit("Загрузка отменена")
                        return
                    time.sleep(0.5)
            ...
            resp = self.session.get(self.url, headers=headers, timeout=REQUEST_TIMEOUT)
            resp.encoding = 'utf-8'
            ...
            manga_id_match = re.search(r'/(\d+)-', self.url)
            if not manga_id_match:
                self.error.emit("Не удалось определить ID манги из ссылки")
                return
            ...
            for i, ch in enumerate(chapters, 1):
                if self._cancelled():
                    return
                chapter_title = ch.get('title') or f"Глава {ch.get('posi', 0)}"
                self.progress.emit(i, total, chapter_title)
                self.status.emit(f"Скачивание: {chapter_title}")
                ...
                arc_resp = self.session.get(archive_url, headers=download_headers, stream=True, timeout=REQUEST_TIMEOUT)
                ...
                with zipfile.ZipFile(temp_zip, 'r') as zf:
                    safe_extract_zip(zf, chapter_folder)
